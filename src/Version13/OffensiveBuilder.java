...
public class OffensiveBuilder {
    ...
    public static void runOffensiveBuilder(RobotController rc) throws GameActionException {
        ...
        if(rc.isActionReady()){
            ...
            if(toHeal != null && rc.canHeal(toHeal.getLocation()) && toHeal.health < 500)
...
//        attemptBuild(rc);
    ...
    public static void runMicroOffensiveBuilder(RobotController rc) throws GameActionException {
        ...
        for(engagementMicroSquare square : options){
            if(square.passable){
                float score = square.enemiesAttackRangedX * -4 + square.enemiesVisiondX * 2 + square.alliesVisiondX * 2.0f + square.alliesHealRangedX + square.potentialEnemiesAttackRangedX * -4;
                if(score > highScore){
                    highScore = score;
                    best = square;
                }
