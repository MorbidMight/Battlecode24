...
//                        MoveAwayFromSpawnLocations(rc);
    ...
    public static MapLocation findClosestSpawnLocation(RobotController rc, MapLocation L) throws GameActionException {
        MapLocation targetLoc = null;
        int distance_1 = L.distanceSquaredTo(Utilities.convertIntToLocation(rc.readSharedArray(0)));
        int distance_2 = L.distanceSquaredTo(Utilities.convertIntToLocation(rc.readSharedArray(1)));
        int distance_3 = L.distanceSquaredTo(Utilities.convertIntToLocation(rc.readSharedArray(2)));
        if (distance_1 < distance_2) {
            if (distance_1 < distance_3) {
                targetLoc = Utilities.convertIntToLocation(rc.readSharedArray(0));
            } else {
